...
- **커스텀 형식 빌더** — 형식을 JSON 파일로 정의해 코드 수정 없이 추가. 내장 형식 3종도 같은 선언·검증 경로를 지난다
- **토큰 예산(선택)** — 게이트마다 누적 사용량을 예산과 대조해 80% 주의·초과 시 종료 권고 (강제 중단 없는 소프트 상한)
- **비교 모드** — 저장된 토론 .md 2개를 골라 메타 대조표(형식·라인업·합의 근접도 궤적·토큰)와 좌우 본문으로 나란히 비교
