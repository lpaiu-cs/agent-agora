...
- **사회자(facilitator) 에이전트** — 선택적 사회자가 단계 경계에서 개회·중간 조율·폐회로 진행을 조율 (토론자가 아닌 부가 레이어)
...
사회자는 완전한 opt-in 이며(`facilitator=null` 이면 동작 불변), 호출이 실패해도
토론을 막지 않는다 — 그 발언은 `phase_records` 가 아닌 `facilitator_notes` 로
누적된다.
