...
│   ├── manager.py         # 토론 상태 관리 및 오케스트레이션 로직
│   ├── database.py        # SQLite 영속성 레이어 (저장/조회)
...
| `AGORA_DB_URL` | (선택) SQLite 접속 URL. 기본 `sqlite:///./agora.db` |
...
## 현재 상태 — phase-5 (DB 이관 + 하이브리드 복붙 터널)

**구현 완료 (전 단계 누적)**

- 전체 데이터 스키마 (`schemas.py`, Pydantic v2) + REST/WebSocket 라우터
- `DiscussionManager` 오케스트레이션:
  - 멀티 공급자 비동기 LLM 호출 + 토큰 단위 스트리밍
  - 동적 프롬프트 조립 / 1·2단계 순차 포스팅 / 우아한 부분 실패 수용
  - 5단계 `force_consensus` 분기 / `request_advance` 게이트 가드
  - 콘텍스트 압축(LTM) — 3단계 이후 1·2단계를 요약 메트릭스로 경량 주입
- 앱 레벨 글로벌 `LLMClientPool` (`lifespan` 이 폐쇄) + 단일 파일 웹 UI
- **phase-5 — DB 이관 + 복붙 터널:**
  - SQLite 영속성 — 토론 상태가 `discussions` 테이블에 저장/조회되며
    서버 재시작 후에도 보존 (`database.py` / `models.py`)
  - `manual` 공급자 — API 미호출, 세션을 `PENDING_MANUAL_INPUT` 으로 격리
    대기시키고 웹 UI 의 딥/일반 복사 + 붙여넣기 창으로 유저 응답을 주입
